...
from cloudmesh.data.api.storage import NotebooksProviderABC
import boto3


class AWSNotebooksProvider(NotebooksProviderABC):
    ...
    def createNoteBook(self, instance_name,instance_type,subnet_id,sg_ids,role_arn,tag_key,tag_value,volume_size_GB):
    ...
    def stopNoteBook(self, instance_name):
        response = client.stop_notebook_instance(
        NotebookInstanceName=instance_name
        )
        return response
    
    def startNoteBook(self, instance_name):
        response = client.start_notebook_instance(
        NotebookInstanceName=instance_name
        )
        return response
    
    def deleteNoteBook(self,instance_name):
        stopNoteBook(instance_name)
